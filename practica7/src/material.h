...
private:
	Material(const TexturePtr& tex, const ShaderPtr& shader, glm::vec4 color, uint8_t shininess)
		: m_texture(tex), m_reflectionTexture(nullptr), m_refractionTexture(nullptr), m_refractionCoef(0.0f), m_normalTexture(nullptr), m_shader(shader), m_color(color), m_shininess(shininess), m_blendMode(ALPHA), m_lighting(true), m_culling(false), m_depthWrite(true){}
	~Material() {}
