...
void Emitter::update(float deltaTime) {
	...
	if (m_isEmitting) {
		...
		for (size_t i = 0; i < particlesToEmit; ++i) {
			...
			MaterialPtr particleMaterial = Material::create(m_material->getTexture(), m_material->getShader());
			particleMaterial->setColor(glm::vec4(glm::linearRand(m_colorRange[0].r, m_colorRange[1].r), glm::linearRand(m_colorRange[0].g, m_colorRange[1].g), glm::linearRand(m_colorRange[0].b, m_colorRange[1].b), glm::linearRand(m_colorRange[0].a, m_colorRange[1].a)));
			particleMaterial->setShininess(m_material->getShininess());
			particleMaterial->setBlendMode(m_material->getBlendMode());
			particleMaterial->setLighting(m_material->getLighting());
			particleMaterial->setCulling(m_material->getCulling());
			particleMaterial->setDepthWrite(m_material->getDepthWrite());
