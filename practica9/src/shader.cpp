...
Shader::Shader(const std::string& vertexShaderSource, const std::string& fragmentShaderSource) : m_Id(0), m_vposLoc(-1) {
	...
	m_vtangentLoc     = glGetAttribLocation(m_Id, "vtangent");
	m_vboneIndicesLoc = glGetAttribLocation(m_Id, "vboneIndices");
	m_vboneWeightsLoc = glGetAttribLocation(m_Id, "vboneWeights");
}
...
void Shader::setupAttribs() const {
	...
	if (m_vtangentLoc != -1) {
		...
		glVertexAttribPointer(m_vtangentLoc, 3, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<const void*>(offsetof(Vertex, tangent)));
	}

	if (m_vboneIndicesLoc != -1) {
		glEnableVertexAttribArray(m_vboneIndicesLoc);
		glVertexAttribPointer(m_vboneIndicesLoc, 4, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<const void*>(offsetof(Vertex, boneIndices)));
	}

	if (m_vboneWeightsLoc != -1) {
		glEnableVertexAttribArray(m_vboneWeightsLoc);
		glVertexAttribPointer(m_vboneWeightsLoc, 4, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<const void*>(offsetof(Vertex, boneWeights)));
	}
}
